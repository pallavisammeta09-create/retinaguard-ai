...
export default function Upload() {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      // If using sample image, fetch it as blob
      if (!file && preview) {
      ...
      fd.append("patient_id", (form.elements.namedItem("patient_id") as HTMLInputElement).value);
      fd.append("age", (form.elements.namedItem("age") as HTMLInputElement).value);
      fd.append("eye", (form.elements.namedItem("eye") as HTMLSelectElement).value);
      const hba1c = (form.elements.namedItem("hba1c") as HTMLInputElement).value;
      const bp    = (form.elements.namedItem("bp") as HTMLInputElement).value;
      const dd    = (form.elements.namedItem("diabetes_duration") as HTMLInputElement).value;
      if (hba1c) fd.append("hba1c", hba1c);
      if (bp)    fd.append("bp", bp);
      if (dd)    fd.append("diabetes_duration", dd);
            ...
            >
              {preview ? (
                <div className="w-full">
                  <div className="relative rounded-2xl overflow-hidden aspect-square max-w-sm mx-auto">
                    ...
                    <button onClick={() => { setPreview(null); setFile(null); }}
                      className="absolute top-2 right-2 h-8 w-8 rounded-full bg-primary text-white grid place-items-center">
                      <X className="h-4 w-4" />
                    </button>
                  </div>
                ...
                <>
                  ...
                  <div className="text-sm text-muted-foreground mt-1">JPEG, PNG up to 50MB</div>
            ...
            <div className="rounded-3xl bg-card border border-border shadow-card p-7">
              ...
              <form className="space-y-4" onSubmit={handleSubmit}>
                <div className="grid grid-cols-2 gap-3">
                  <div><Label>Patient ID</Label><Input name="patient_id" className="mt-1.5" placeholder="P-00482" required /></div>
                  <div><Label>Age</Label><Input name="age" className="mt-1.5" type="number" placeholder="58" required /></div>
                </div>
                <div className="grid grid-cols-2 gap-3">
                  <div><Label>HbA1c (%)</Label><Input name="hba1c" className="mt-1.5" placeholder="8.4" /></div>
                  <div><Label>BP (mmHg)</Label><Input name="bp" className="mt-1.5" placeholder="142/88" /></div>
                </div>
                <div>
                  <Label>Diabetes duration (years)</Label>
                  <Input name="diabetes_duration" className="mt-1.5" type="number" placeholder="12" />
                </div>
                <div>
                  <Label>Eye</Label>
                  <select name="eye" className="mt-1.5 flex h-10 w-full rounded-md border border-input bg-background px-3 text-sm">
                    <option value="OD">OD (Right)</option>
                    <option value="OS">OS (Left)</option>
                  </select>
                </div>
                <Button type="submit" disabled={!preview || loading} size="lg" className="w-full bg-gradient-accent text-white hover:opacity-90">
                  {loading ? <><Loader2 className="h-4 w-4 mr-2 animate-spin" /> Analyzing...</> : <><Sparkles className="h-4 w-4 mr-2" /> Analyze Scan</>}
                </Button>
              </form>
            </div>
          </div>
